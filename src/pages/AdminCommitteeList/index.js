...
function AdminCommitteeList({ route, navigation }) {
  ...
  const getUsersCommitteeAPI = () => {
    setLoadingScreen(true);
    CallAPI({
      url: `${API_HOST}/point-transaction/status?type=committee&eventId=${eventId}&status=true`,
      method: 'GET',
      data: null,
    })
      .then((r) => {
  ...
  return (
    <View style={{ flex: 1 }} onLayout={onLayoutRootView}>
      <NavbarTop title="Data Panitia" />
      <View style={styles.wrapper}>
        <View style={{ paddingHorizontal: 35 }}>
          <View style={{ paddingVertical: 20 }}>
            <SearchBar placeholder="cari nama partisipan" />
          </View>
          <Text style={styles.titleText}>Daftar Panitia</Text>
        </View>
        ...
        <View style={{ paddingHorizontal: 35, marginVertical: 10 }}>
          <PrimaryButton title="Ubah Panitia" onPress={() => navigation.navigate('AdminAddCommittee', { eventId })} />
        </View>
      </View>
